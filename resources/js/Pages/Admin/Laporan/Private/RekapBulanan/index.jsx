...
const Laporan = ({
    ...
    const downloadExcelGabungan = (laporanPrivate, laporanPengeluaranPrivate, paketPrivate, judul) => {
        ...
        const headerPengeluaran = ["Pembuat Laporan", "Detail Gaji Private", "ATK", "Sewa", "Intensif", "Lisensi", "THR", "Lain Lain", "Jumlah", "Albri"];
    ...
    // Fungsi untuk print laporan private
    const printLaporan = () => {
        ...
        let printContent = `
            ...
            <html>
            <head>
                <title>Laporan Rekap Bulanan Private ${bulan.toUpperCase()} ${tahun}</title>
                <style>
                    body { 
        ...
        // Add paket headers
        safePaketPrivate.forEach(paket => {
        ...
        safePaketPrivate.forEach(p => { totalPaketPrint[p.id] = 0; });

        if (laporanPrivate && laporanPrivate.data) {
            laporanPrivate.data.forEach(laporan => {
                printContent += `
                    <tr>
                        <td>${laporan.hari}</td>
                ...
                safePaketPrivate.forEach(paket => {
                    const jumlah = laporan.pakets ? (laporan.pakets[paket.id] ? parseInt(laporan.pakets[paket.id]) : 0) : 0;
        ...
        if (laporanPengeluaranPrivate && laporanPengeluaranPrivate.data) {
            laporanPengeluaranPrivate.data.forEach(pengeluaran => {
                ...
                // Handle different gaji detail structures
                if (pengeluaran.gurus && Array.isArray(pengeluaran.gurus) && pengeluaran.gurus.length > 0) {
                    gajiDetail = pengeluaran.gurus.map(guru => `${guru.guru_id}: Rp ${(guru.gaji || 0).toLocaleString()}`).join('<br>');
                } else if (pengeluaran.gaji && n(pengeluaran.gaji) > 0) {
                    const namaGuru = pengeluaran.user?.name || 'Private';
                    gajiDetail = `${namaGuru}: Rp ${n(pengeluaran.gaji).toLocaleString()}`;
                }
        ...
        printContent += `
                <tr class="total-row">
                    <td colspan="3"><strong>TOTAL</strong></td>
                    <td><strong>Total Gaji: Rp ${totalGajiSemuaPrivate.toLocaleString()}</strong></td>
